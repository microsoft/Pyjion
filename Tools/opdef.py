opdefs = """OPDEF(CEE_NOP,                        "nop",              Pop0,               Push0,       InlineNone,         IPrimitive,  1,  0xFF,    0x00,    NEXT)
...
opcodes = opdefs.split("\n")
for line in opcodes:
    line = line[6:-1].replace(' ', '')  # strip opdef wording and padding
    op, name, se, se2, type, char, bytes_, first, second, _ = line.split(',', 10)
    print(f'case {op}: sprintf(out, "%20s {se} / {se2}", {name}); break;\n')
